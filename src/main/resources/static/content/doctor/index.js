...
$(function () {
    ...
    table.render({
        ...
        even: false,// 隔行背景
        skin: 'line',// 行边框风格
        url: SERVER_ROUTER.PATH_QUERY_PAGE_DOCTOR + '?hosId=' + hosId,
        method: 'GET',
        page: true,
        limits: [10, 20, 50],
        limit: 10,
...
var fillDoctor = function () {
    $.ajax({
        url: SERVER_ROUTER.PATH_QUERY_PAGE_DOCTOR,
        data: {
            currentPage: 1,
            limit: 1000000
        },
...
var save = function () {
    ...
    $.ajax({
        url: SERVER_ROUTER.PATH_DOCTOR_UPDATE + '/' + id,
        type: 'PUT',
        dataType: 'JSON',
        data: data,
        success: function (data) {
...
var del = function (obj, index) {
    var data = obj.data;
    $.ajax({
        url: SERVER_ROUTER.PATH_DOCTOR_UPDATE + '/' + data.id,
        type: 'DELETE',
        dataType: 'JSON',
        success: function (data) {
            if (data && data.status == SERVER_RESPONSE_CODE.OK) {
